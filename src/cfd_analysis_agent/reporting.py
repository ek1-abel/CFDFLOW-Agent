...
def extract_report_and_telemetry(result_text: str) -> ReportExtractionResult:
    if not result_text.strip():
        return ReportExtractionResult(
            report_markdown="# CFD Post-Processing Analysis Report\n\nNo valid output was produced.",
    ...
    if telemetry_match:
        ...
        try:
            ...
            telemetry = ReportTelemetry(
                ...
                domain=str(payload.get("domain", "CFD_post_processing")).strip() or "CFD_post_processing",
                ...
                warning=None,
                raw_payload=payload,
                cfd_task_type=str(payload.get("cfd_task_type", "")).strip(),
                convergence_status=str(payload.get("convergence_status", "")).strip(),
            )
    ...
    if not cleaned_report:
        cleaned_report = "# CFD Post-Processing Analysis Report\n\nNo valid Markdown report body was produced."
