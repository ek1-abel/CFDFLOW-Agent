"""Dataset metadata extraction with CFD task classification."""
...
from .cfd_classifier import classify_cfd_task, get_tool_recommendation
...
class DataContextSummary:
    ...
    cfd_recommended_tool: str = "PythonInterpreterTool"
    cfd_matched_columns: dict[str, list[str]] | None = None
...
def build_data_context(
    ...
    *,
    ...
    """Build a compact metadata-first prompt context for a local CFD dataset."""
    ...
    # CFD task classification
    cfd_task_type, cfd_matched_columns, cfd_recommended_tool = classify_cfd_task(columns)

    sample_size_warning = ""
    small_sample_warning = shape[0] < 30
    if small_sample_warning:
        sample_size_warning = (
            "WARNING: 当前数据量较小 (N<30)，分析结论的统计可信度有限。"
        )
    ...
    # CFD classification info
    context_lines.append(f"CFD任务类型: {cfd_task_type}")
    context_lines.append(f"推荐分析工具: {cfd_recommended_tool}")
    if cfd_matched_columns:
        matched_str = ", ".join(f"{k}: {v}" for k, v in cfd_matched_columns.items())
        context_lines.append(f"匹配列: {matched_str}")
    ...
    if pdf_small_table_mode:
        context_lines.append(
            ...
            "This is a PDF-derived small results table from CFD comparison or validation studies.\n"
            "Use a lightweight analysis template.\n"
            "</PDF_Small_Table_Mode>"
        )
    ...
    return DataContextSummary(
        ...
        cfd_task_type=cfd_task_type,
        cfd_recommended_tool=cfd_recommended_tool,
        cfd_matched_columns=dict(cfd_matched_columns) if cfd_matched_columns else None,
    )
