"""Custom agent runner and scientific ReAct controller for CFD post-processing."""
...
def build_plaintext_event_handler() -> EventHandler:
    ...
    def handle_event(event_type: str, payload: dict[str, Any]) -> None:
        if event_type == "config_loading":
            ...
            cfd_task = payload.get("cfd_task_type", "unknown")
            print(f"      Data shape: {shape[0]} rows x {shape[1]} columns")
            print(f"      CFD task type: {cfd_task}")
...
def build_tool_registry(*, enable_search: bool = True) -> ToolRegistry:
    """Create the tool registry for the CFD analysis agent."""
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        ...
        tool_registry.register_tool(ResidualAnalysisTool())
        tool_registry.register_tool(ForceAnalysisTool())
        tool_registry.register_tool(PressureAnalysisTool())
        tool_registry.register_tool(VelocityAnalysisTool())
        tool_registry.register_tool(GridStudyTool())
...
def _build_observation_summary(
    *,
    ...
    if tool_name in _CFD_TOOL_NAMES and isinstance(data, dict):
        analysis_text = _truncate_text(str(data.get("analysis", text)).strip(), 1500)
        figures = data.get("figures_saved", [])
        convergence = str(data.get("convergence_status", "")).strip()
        if analysis_text:
            parts.append(f"CFD Analysis Result:\n{analysis_text}")
        if convergence:
            parts.append(f"Convergence Status: {convergence}")
        if isinstance(figures, list) and figures:
            parts.append("Figures saved: " + ", ".join(str(f) for f in figures))
        return "\n\n".join(parts)
...
def _build_step_summary(tool_name: str, decision: str, tool_status: str, observation_preview: str) -> str:
    tool_action_labels = {
        "TavilySearchTool": "Online domain knowledge retrieval",
        "PythonInterpreterTool": "Local Python execution",
        "ResidualAnalysisTool": "CFD residual convergence analysis",
        "ForceAnalysisTool": "CFD force coefficient analysis (Cl/Cd/Cm)",
        "PressureAnalysisTool": "CFD pressure distribution analysis (Cp)",
        "VelocityAnalysisTool": "CFD velocity profile analysis",
        "GridStudyTool": "CFD grid independence study (Richardson/GCI)",
    }
    action_text = tool_action_labels.get(tool_name, f"Tool call: {tool_name}")
    ...
    *,
    ...
    payload = {
        ...
        "telemetry": {
            ...
            "cfd_task_type": telemetry.cfd_task_type,
            "convergence_status": telemetry.convergence_status,
        },
        "artifact_validation": {
...
class ScientificReActRunner:
    """Custom JSON-driven ReAct controller for CFD analysis tasks."""
    ...
    def run_with_messages(
        ...
        *,
        ...
        fallback_report = (
            "# CFD Post-Processing Analysis Report\n\n"
            ...
            "tool execution errors, or insufficient CFD analysis instructions.\n\n"
            "<telemetry>{\"methods\": [], \"domain\": \"CFD_post_processing\", \"tools_used\": [], "
            "\"search_used\": false, \"search_notes\": \"Agent reached max steps before finalizing.\", "
            "\"cleaned_data_saved\": false, \"cleaned_data_path\": \"\", \"figures_generated\": [], "
            "\"cfd_task_type\": \"\", \"convergence_status\": \"\"}</telemetry>"
        )
...
def run_analysis(
    ...
    *,
    ...
    agent_name: str = "CFD Post-Processing Analyst",
    max_steps: int = 6,
    ...
    """Run the full CFD post-processing analysis workflow."""
